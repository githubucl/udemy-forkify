...
class RecipeView extends View {
  ...
  addHandlerRender(controlRecipe) {
    ...
    event.forEach(ev => window.addEventListener(ev, controlRecipe))
  }

  addHandlerRenderUpdate(handler) {
    this._parentElement.addEventListener('click', function (e) {
      const button = e.target.closest('.btn--increase-servings');
      if (!button) return
      const buttserving = +button.dataset.updates
...
<div class="recipe__info">
  ...
  <div class="recipe__info-buttons">
    <button class="btn--tiny btn--increase-servings" data-updates='${this._data.servings > 1 ? this._data.servings - 1 : this._data.servings}'>
      <svg>
        <use href="${icons}#icon-minus-circle"></use>
      </svg>
    </button>
    <button class="btn--tiny btn--increase-servings" data-updates='${this._data.servings + 1}'>
      <svg>
...
// const recipeList = document.querySelector('.recipe__ingredient-list')

// recipe.ingredients.forEach(item => {
//     let quantity, unit, description;
